...
class AudioRecorder:
    ...
    def calculate_levels(self, audio_data):
        ...
        try:
            ...
            # Convert to positive scale (0 dB = max, -96 dB = silence)
            # Shift to make it more intuitive: add DB_OFFSET to get calibrated range
            rms_db_shifted = max(0, rms_db + DB_OFFSET)
            peak_db_shifted = max(0, peak_db + DB_OFFSET)
        ...
        try:
            ...
            for i in range(0, numdevices):
                ...
                if dev_info.get('maxInputChannels') > 0:
                    print(f"  Device {i}: {dev_info.get('name')} (Max Channels: {dev_info.get('maxInputChannels')})")
        ...
        for channels in channels_to_try:
            try:
                ...
                if device_index is not None:
                    ...
                    print(f"Attempting to open audio: Device={device_index}, Channels={channels}, Rate={RATE}")
                else:
                    print(f"Attempting to open audio: Default Device, Channels={channels}, Rate={RATE}")
                
                self.stream = self.p.open(**stream_params)
                threading.Thread(target=self.monitor_loop, daemon=True).start()
                print(f"✓ Audio monitoring started successfully with {channels} channel(s).")
                return  # Success!
    ...
    def save_recording(self):
        ...
        try:
            ...
            print(f"Saved audio: {filepath} (Frames: {len(self.recording_frames)}, Peak: {self.max_db_recorded:.1f}dB)")
            return filepath
